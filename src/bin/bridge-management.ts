...
const MALFORMED_CAUSE_MESSAGES = {
  ...
  ERR_BRIDGE_ENCODING:
    "The companion sent malformed authenticated protocol data.",
  ERR_BRIDGE_PAYLOAD:
    "The companion sent malformed authenticated protocol data.",
  ERR_BRIDGE_VERSION_DATA:
    "The companion returned invalid authenticated version data.",
};

function causeMessage(failure) {
  return failure.cause instanceof Error
    ? MALFORMED_CAUSE_MESSAGES[failure.cause.code]
    : undefined;
}
...
function managementFailure(error) {
  ...
  if (error.kind === "authentication")
    return new CliError("The companion response could not be authenticated.");
  if (error.kind === "transport") {
    return new CliError(
      error.stage === "connect" || error.stage === "request-write"
        ? "The companion Unix socket is unavailable."
        : "The companion closed an incomplete health response."
    );
  }
  if (error.stage === "connect")
    return new CliError("The companion Unix socket is unavailable.");
  if (error.stage === "request-write")
    return new CliError(CREDENTIAL_REJECTION);
  // Strict JSON and UTF-8 faults in a frame keep a native decoding cause; shape faults carry none.
  if (error.cause instanceof Error)
    return new CliError("The companion sent malformed protocol data.");
  return new CliError(
...
export async function companionRequestAt(
  ...
  operation,
  fixedRequestId,
  protocol = productionBridgeProtocol
) {
  ...
  try {
    response = await protocol.request({
      ...
      timing: {
        ...
        connect: {
          kind: "absolute",
          at: startedAt + MANAGEMENT_PHASE_TIMEOUT_MS,
        },
        challenge: {
          kind: "absolute",
          at: startedAt + MANAGEMENT_PHASE_TIMEOUT_MS,
...
export async function healthAt(
  endpoint,
  credential,
  protocol = productionBridgeProtocol
) {
  const health = await companionRequestAt(
    endpoint,
    credential,
    "health",
    undefined,
  ...
  if (
    !exactObject(health, ["permission", "defaultInputAvailable"]) ||
