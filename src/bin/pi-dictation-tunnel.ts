...
import { createBoundedLogger } from "./bounded-log.js";
...
if (!configurationPath)
  throw new Error("Tunnel supervisor configuration is required.");
...
const argumentListIsSafe = (value) =>
  Array.isArray(value) &&
...
if (
  configuration.product !== "com.yasuhito.pi-dictation.bridge" ||
  typeof configuration.logFile !== "string" ||
  Buffer.byteLength(configuration.logFile) > 4096 ||
  ...
  !Number.isSafeInteger(configuration.stableAfterMs) ||
  configuration.stableAfterMs < 1000 ||
  configuration.stableAfterMs > 300000
) {
...
function writeStatus(patch) {
  ...
  const next = {
    ...current,
    stages: { ...current.stages, ...patch },
    updatedAt: new Date().toISOString(),
  };
  const temporary = join(
    dirname(configuration.statusFile),
    `.${basename(configuration.statusFile)}.${process.pid}.tmp`
  );
...
async function probeTunnel(tunnel) {
  while (
    ...
    if (!listener.error && listener.status === 0) {
      ...
      writeStatus({
        authenticatedHealth:
          !health.error && health.status === 0 ? "ready" : "pending",
      });
    } else {
...
while (!stopping) {
  ...
  if (cleanup.error || cleanup.status !== 0) {
    writeStatus({
      tunnelProcess: "stopped",
      listener: "pending",
      authenticatedHealth: "pending",
    });
    failures = Math.min(failures + 1, 6);
    logger.event("listener-cleanup-failed", {
      stage: "connect",
      retry: failures,
    });
    ...
    await sleep(
      Math.min(
        ...
        Math.max(
          250,
          ceiling +
            randomInt(-Math.floor(ceiling / 4), Math.floor(ceiling / 4) + 1)
        )
      )
    );
    continue;
  ...
  failures =
    lifetime >= configuration.stableAfterMs ? 0 : Math.min(failures + 1, 6);
  ...
  const jittered = Math.min(
    ...
    Math.max(
      250,
      ceiling + randomInt(-Math.floor(ceiling / 4), Math.floor(ceiling / 4) + 1)
    )
  );
