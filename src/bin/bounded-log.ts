...
function safeText(value, fallback) {
  const text =
    typeof value === "string" && SAFE_TEXT.test(value) ? value : fallback;
...
export function createBoundedLogger(path, component) {
  ...
  const append = (record) => {
    ...
    if (bytes.length > 1024)
      throw new Error("Refusing oversized bridge log record.");
  ...
  const flushRepeats = () => {
    if (repeats > 0)
      append({ component: safeComponent, code: "repeated", count: repeats });
    repeats = 0;
  };
  return {
    event(
      code,
      fields: { stage?: unknown; retry?: number; version?: number } = {}
    ) {
      const record = {
        ...
        ...(fields.stage === undefined
          ? {}
          : { stage: safeText(fields.stage, "unknown") }),
        ...(Number.isSafeInteger(fields.retry)
          ? { retry: Math.max(0, Math.min(fields.retry, 1_000_000)) }
          : {}),
        ...(Number.isSafeInteger(fields.version)
          ? { version: Math.max(0, Math.min(fields.version, 1_000_000)) }
          : {}),
      };
      ...
      if (key === previousKey) {
        repeats = Math.min(repeats + 1, 1_000_000);
        return;
      }
      flushRepeats();
      append(record);
...
export const boundedLogPolicy = Object.freeze({
  generations: 3,
  bytesPerGeneration: MAX_LOG_BYTES,
});
