...
import { isCanonicalIdentity } from "../lib/bridge-protocol.js";
...
const sourcePath = join(
  ...
  "native",
  "macos-companion",
  "PiDictationBridge.swift"
);
const watchdogSourcePath = join(
  packageRoot,
  "native",
  "macos-companion",
  "PiDictationDurationWatchdog.swift"
);
...
function inspectPath(path, kind, mode, description = "managed artifact") {
  ...
  if (stat.isSymbolicLink())
    throw new CliError(`Refusing symlink at ${description}.`);
  const expected = kind === "directory" ? stat.isDirectory() : stat.isFile();
  if (!expected)
    throw new CliError(`Refusing unexpected file type at ${description}.`);
  if (uid() !== undefined && stat.uid !== uid()) {
    throw new CliError(
      `Refusing ${description} not owned by the current user.`
    );
  }
  if ((stat.mode & 0o777) !== mode) {
    throw new CliError(
      `Refusing ${description} with unsafe permissions (expected ${mode.toString(8)}).`
    );
  }
...
function inspectSocket(path) {
  ...
  try {
    ...
    if (error?.code === "ENOENT")
      throw new CliError("The companion Unix socket is unavailable.");
    throw error;
  }
  if (stat.isSymbolicLink() || !stat.isSocket())
    throw new CliError(
      "Refusing unexpected file type at companion Unix socket."
    );
  if (uid() !== undefined && stat.uid !== uid())
    throw new CliError(
      "Refusing a companion Unix socket not owned by the current user."
    );
  if ((stat.mode & 0o777) !== 0o600)
    throw new CliError(
      "Refusing a companion Unix socket with unsafe permissions."
    );
}
...
function ensureManagedParents(p) {
  ...
  if (!existsSync(library)) mkdirSync(library, { mode: 0o700 });
    ...
    if (stat.isSymbolicLink())
      throw new CliError("Refusing symlink at Library directory.");
  ...
  for (const parent of [
    ...
    if (stat.isSymbolicLink())
      throw new CliError(`Refusing symlink at ${basename(parent)} directory.`);
    if (!stat.isDirectory() || (uid() !== undefined && stat.uid !== uid())) {
      throw new CliError(
        `Refusing unowned or unexpected ${basename(parent)} directory.`
      );
    }
  }
  ensureDirectory(p.productRoot, 0o700, "Pi Dictation support directory");
  ensureDirectory(
    join(p.home, "Library", "Caches", "pi-dictation"),
    0o700,
    "Pi Dictation cache directory"
  );
}
...
function run(
  command,
  args,
  options: { exposeOutput?: boolean; failure?: string } = {}
) {
  const result = spawnSync(command, args, { encoding: "utf8", ...options });
  if (result.error || result.status !== 0) {
    const detail = options.exposeOutput
...
function validateToolchain() {
  ...
  try {
    swiftc = run("xcrun", ["--find", "swiftc"], {
      failure:
        "Swift compiler not found. Install Apple's command line tools with: xcode-select --install",
    });
  } catch (error) {
  ...
  const version = run(swiftc, ["--version"], {
    failure:
      "Swift is unusable. Update Xcode or Apple's command line tools and run xcode-select --switch.",
  });
  const match = version.match(/Swift version (\d+)\.(\d+)/i);
  if (
    !match ||
    Number(match[1]) < 5 ||
    (Number(match[1]) === 5 && Number(match[2]) < 9)
  ) {
    throw new CliError(
      "Swift 5.9 or newer is required. Update Xcode or Apple's command line tools."
    );
  }
  if (!/Target:\s*arm64-apple-macosx/i.test(version)) {
    throw new CliError(
      "Bridge companion builds require an Apple Silicon Mac and a macOS Swift toolchain. Run xcode-select --switch with a current Xcode toolchain."
    );
  }
  const sdk = run("xcrun", ["--sdk", "macosx", "--show-sdk-path"], {
    failure:
      "The macOS SDK is unavailable. Update Xcode or Apple's command line tools.",
  });
  try {
    run("codesign", ["--version"], {
      failure: "codesign is unavailable. Install Apple's command line tools.",
    });
  } catch (error) {
    // Apple's codesign prints its version to stderr on some releases; existence is proven later by signing.
    if (
      error instanceof CliError &&
      !process.env.PATH?.split(":").some((entry) =>
        existsSync(join(entry, "codesign"))
      )
    )
      throw error;
  }
...
function buildBundle(output, installId = "standalone-build") {
  if (!existsSync(sourcePath) || !existsSync(watchdogSourcePath)) {
    throw new CliError(
      "The packaged macOS companion source is missing. Reinstall pi-dictation."
    );
  }
  if (existsSync(output))
    throw new CliError("Refusing to replace an existing build output.");
  const toolchain = validateToolchain();
  const parent = dirname(output);
  if (!existsSync(parent))
    throw new CliError("Build output parent directory does not exist.");
  const stage = join(parent, `.${basename(output)}.${randomUUID()}.stage`);
  const entitlements = join(
    parent,
    `.pi-dictation-entitlements.${randomUUID()}.plist`
  );
  try {
    ...
    run(
      ...
      {
        failure:
          "The Swift toolchain cannot build the macOS companion. Update Xcode or Apple's command line tools.",
      }
    );
    ...
    run(
      ...
      {
        failure:
          "The Swift toolchain cannot build the duration watchdog. Update Xcode or Apple's command line tools.",
      }
    );
    chmodSync(watchdog, 0o700);
    writeFileSync(
      join(stage, "Contents", "Info.plist"),
      infoPlist(packageVersion()),
      { mode: 0o600 }
    );
    writeFileSync(
      join(resources, "ownership.json"),
      JSON.stringify({ product: LABEL, installId }) + "\n",
      { mode: 0o600 }
    );
    writeFileSync(entitlements, entitlementsPlist(), { mode: 0o600 });
    run(
      "codesign",
      ["--force", "--sign", "-", "--entitlements", entitlements, stage],
      {
        failure:
          "The companion could not be signed. Update Xcode or Apple's command line tools.",
      }
    );
...
function validateCredential(path) {
  ...
  if (
    ...
    !/^[A-Za-z0-9+/]{43}=$/.test(credential.secret) ||
...
function existingInstallId(p) {
  ...
  if (receipt.product !== LABEL || !isCanonicalIdentity(receipt.installId)) {
    throw new CliError(
      "Refusing bridge artifacts whose ownership cannot be proven."
    );
  }
...
function proveApp(path, installId) {
  ...
  inspectPath(
    join(macos, "PiDictationDurationWatchdog"),
    "file",
    0o700,
    "duration watchdog executable"
  );
  const marker = readJsonOwned(
    join(resources, "ownership.json"),
    "companion ownership marker"
  );
  if (marker.product !== LABEL || marker.installId !== installId) {
    throw new CliError(
      "Refusing a companion app whose ownership cannot be proven."
    );
  }
}
...
function install() {
  if (uid() === 0)
    throw new CliError(
      "Refusing to install as root. Run as the logged-in Mac user without sudo."
    );
  const p = paths();
  ...
  try {
    ...
    if (pathExists(p.preflight)) {
      ...
      if (previousPreflight.installId !== installId)
        throw new CliError("Refusing an unowned preflight receipt.");
    }
    if (pathExists(p.socket)) {
      inspectSocket(p.socket);
      throw new CliError(
        "The companion is running. Stop its user LaunchAgent before reinstalling."
      );
    }
    ...
    if (pathExists(p.plist)) {
      ...
      if (
        !readFileSync(p.plist, "utf8").includes(
          `pi-dictation-install-id:${installId}`
        )
      ) {
        throw new CliError(
          "Refusing a LaunchAgent whose ownership cannot be proven."
        );
      }
    }
    ...
    chmodSync(
      join(stagedApp, "Contents", "MacOS", "PiDictationDurationWatchdog"),
      0o700
    );
    chmodSync(
      join(stagedApp, "Contents", "Resources", "ownership.json"),
      0o600
    );
    ...
    console.log(
      "Run `pi-dictation bridge preflight` interactively to verify microphone permission and real audio."
    );
  } finally {
...
function verifyInstallation() {
  ...
  if (receipt.product !== LABEL || !isCanonicalIdentity(receipt.installId)) {
    throw new CliError(
      "Bridge ownership cannot be proven. Run `pi-dictation bridge install`."
    );
  }
  ...
  if (
    !readFileSync(p.plist, "utf8").includes(
      `pi-dictation-install-id:${receipt.installId}`
    )
  ) {
    throw new CliError(
      "Refusing a LaunchAgent whose ownership cannot be proven."
...
async function preflight() {
  ...
  if (pathExists(p.preflight)) {
    ...
    if (previous.installId !== receipt.installId)
      throw new CliError("Refusing an unowned preflight receipt.");
  }
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    ...
    throw new CliError(
      "Preflight requires an interactive terminal in the logged-in Mac GUI session."
    );
  }
  console.log(
    "Speak naturally for five seconds when macOS asks for microphone access."
  );
  const resultPath = join(p.runtime, `preflight-${randomUUID()}.json`);
  try {
    run(
      "open",
      ["-gj", "-W", p.app, "--args", "--preflight-result", resultPath],
      {
        failure:
          "The installed companion could not run preflight in the GUI session",
      }
    );
    ...
    if (permission !== "authorized" || result.capture !== "observed") {
      throw new CliError(
        "Bridge preflight did not observe usable microphone audio; check Privacy & Security > Microphone and the selected input device."
      );
    }
    atomicWrite(
      p.preflight,
    ...
    try {
      run("launchctl", ["bootstrap", `gui/${uid()}`, p.plist], {
        failure:
          "Preflight passed, but the user LaunchAgent could not be loaded. Log out and back in, then rerun preflight",
      });
    ...
    console.log(
      "Pi Dictation Bridge preflight passed; the companion is ready."
    );
    ...
    if (pathExists(upgradePath)) {
      ...
      if (
        upgradeState.product !== LABEL ||
        upgradeState.phase !== "preflight-required" ||
        !Array.isArray(upgradeState.hosts)
      ) {
      ...
      try {
        ...
        if (
          JSON.stringify(checkedAliases) !==
          JSON.stringify([...upgradeState.hosts].sort())
        ) {
          throw new CliError(
            "Registered bridge destinations changed during upgrade; rerun upgrade checks."
          );
        }
        ...
        if (
          healthReports.some(
            (host) =>
              host.listener.status !== "established" ||
              host.authenticatedHealth.status !== "ready" ||
              host.protocolCompatibility.status !== "compatible"
          )
        ) {
          throw new CliError(
            "Upgrade preflight passed, but not every registered destination passed listener and authenticated health checks. Repair the affected bridge and rerun preflight."
          );
        }
        rmSync(upgradePath);
        console.log(
          "Upgrade complete; authenticated health passed for every registered destination."
        );
      } catch (error) {
        spawnSync("launchctl", ["bootout", `gui/${uid()}/${LABEL}`], {
          stdio: "ignore",
...
async function authenticatedHealth() {
  ...
  if (pathExists(join(p.root, "upgrade.json"))) {
    throw new CliError(
      "Bridge upgrade is incomplete; finish required real-audio preflight and all-host health checks first."
    );
  }
  const ready = readJsonOwned(p.preflight, "preflight receipt");
  if (
    ready.product !== LABEL ||
    ready.installId !== receipt.installId ||
    ready.executableSha256 !== executableDigest(p)
  ) {
    throw new CliError(
      "The installed build has not passed real-audio preflight."
    );
  }
  inspectSocket(p.socket);
  ...
  console.log(
    `Default input: ${health.defaultInputAvailable ? "available" : "unavailable"}`
  );
}
...
function build(args) {
  ...
  if (index === -1 || !args[index + 1] || args.length !== 2) {
    throw new CliError(
      "Usage: pi-dictation bridge build --output /path/to/PiDictationBridge.app"
    );
  }
...
function diagnosticStage(status, detail = undefined) {
...
async function bridgeDoctor(json) {
  ...
  if (pathExists(p.root)) {
    try {
      ...
      companionLaunchAgent = diagnosticStage(
        launchAgentLoaded(LABEL) ? "loaded" : "not-loaded"
      );
    } catch {
      installation = diagnosticStage("needs-attention");
      companionLaunchAgent = pathExists(p.plist)
        ? diagnosticStage("needs-attention")
        : diagnosticStage("not-configured");
    }
  }
  if (pathExists(p.socket)) {
    try {
      inspectSocket(p.socket);
      companionProcess = diagnosticStage("socket-present");
    } catch {
      companionProcess = diagnosticStage("needs-attention");
    }
  }
  let hosts = [];
  if (installation.status !== "not-installed") {
    try {
      hosts = await diagnoseHosts(companionRequestAt);
    } catch {
      hosts = [];
    }
  }
  if (hosts.some((host) => host.authenticatedHealth.status === "ready"))
    companionProcess = diagnosticStage("running");
  let permission =
    hosts.find((host) => host.permission.status !== "unavailable")
      ?.permission || diagnosticStage("unavailable");
  if (
    permission.status === "unavailable" &&
    installation.status === "ready" &&
    companionProcess.status === "running"
  ) {
    try {
      const credential = validateCredential(p.credential);
      const health = await healthAt(
        { type: "unix", path: p.socket },
        credential
      );
  ...
  const report = {
    ...
    companion: {
      installation,
      permission,
      launchAgent: companionLaunchAgent,
    ...
    limits: {
      ...
      maximumConnectionsPerCredential: 4,
      maximumRetainedWav: 2,
      maximumRetainedWavBytes: 268435456,
      levelHistoryObservations: 600,
    },
  };
  const encoded = JSON.stringify(report);
  if (Buffer.byteLength(encoded) > 1024 * 1024)
    throw new CliError("Bridge doctor output exceeds its safe limit.");
  ...
  for (const host of hosts) {
    ...
    console.log(
      `  Protocol compatibility: ${host.protocolCompatibility.status}`
    );
...
function bridgeLogs(alias) {
  ...
  if (records.length === 0)
    return console.log("No bridge log records available.");
  const lines = records.map(
    (record) =>
      `${record.source} ${record.component} ${record.code}${record.stage ? ` stage=${record.stage}` : ""}${record.retry === undefined ? "" : ` retry=${record.retry}`}${record.version === undefined ? "" : ` version=${record.version}`}`
  );
  const output = lines.join("\n");
  if (Buffer.byteLength(output) > 64 * 1024)
    throw new CliError("Bridge log output exceeds its safe limit.");
  console.log(output);
}
...
async function bridgeRepair(args) {
  ...
  if (
    positional.length > 1 ||
    args.some((arg) => arg.startsWith("--") && arg !== "--confirm")
  ) {
    throw new CliError(
      "Usage: pi-dictation bridge repair [ssh-alias] [--confirm]"
    );
  }
  const initialPaths = paths();
  if (!pathExists(initialPaths.root)) {
    console.log(
      "Repair preview: no installed bridge artifacts; no changes are required."
    );
    console.log(
      "Preview only. Repair never installs, changes credentials, requests permission, or opens audio."
    );
    return;
  }
  const { p, receipt } = verifyInstallation();
  ...
  if (aliases.some((alias) => !known.includes(alias)))
    throw new CliError("Unknown host bridge; no repair was performed.");
  ...
  const reconcileAliases = new Set(
    reports
      .filter(
        (host) =>
          host.tunnelProcess.status !== "running" ||
          host.listener.status !== "established" ||
          host.authenticatedHealth.status !== "ready" ||
          host.protocolCompatibility.status !== "compatible"
      )
      .map((host) => host.sshAlias)
  );
  for (const alias of aliases)
    repairHost(alias, false, reconcileAliases.has(alias));
  if (!confirmed) {
    if (!reloadCompanion && aliases.length === 0)
      console.log("Repair preview: no changes are required.");
    console.log(
      "Preview only. Rerun with --confirm. Credentials, microphone permission, and audio are never changed."
    );
    return;
  }
  if (reloadCompanion) {
    ...
    if (!loaded)
      run("launchctl", ["bootstrap", domain, p.plist], {
        failure: "The companion LaunchAgent could not be loaded",
      });
    run(
      "launchctl",
      ["kickstart", ...(loaded ? ["-k"] : []), `${domain}/${LABEL}`],
      { failure: "The companion could not be restarted" }
    );
  }
  for (const alias of aliases)
    repairHost(alias, true, reconcileAliases.has(alias));
}
...
async function upgrade(args) {
  ...
  if (args.some((arg) => !allowed.has(arg)))
    throw new CliError(
      "Usage: pi-dictation bridge upgrade [--cancel-active --confirm]"
    );
  ...
  if (pathExists(upgradePath)) {
    ...
    if (
      upgradeState.product !== LABEL ||
      !["quiescing", "ready-to-install"].includes(upgradeState.phase) ||
      !Array.isArray(upgradeState.hosts) ||
      !Array.isArray(upgradeState.cancelAliases) ||
      !upgradeState.requestIds ||
      typeof upgradeState.requestIds !== "object"
    ) {
      throw new CliError(
        "Upgrade already changed the companion; complete required real-audio preflight before continuing."
      );
    }
  } else {
    ...
    const active = effects
      .filter(
        (effect) =>
          effect.activeRecordingLease > 0 || effect.incompleteAudio > 0
      )
      .map((effect) => effect.sshAlias);
    if (active.length && !args.includes("--cancel-active")) {
      throw new CliError(
        `Active recording blocks upgrade for: ${active.join(", ")}. Rerun with --cancel-active --confirm to delete affected audio.`
      );
    }
    ...
    upgradeState = {
      ...
      phase: "quiescing",
      hosts,
      cancelAliases: active,
      requestIds: Object.fromEntries(
        hosts.map((alias) => [alias, randomUUID()])
      ),
    };
    atomicWrite(upgradePath, `${JSON.stringify(upgradeState)}\n`);
  }
  if (upgradeState.phase === "quiescing") {
    if (upgradeState.cancelAliases.length)
      console.log(
        `Cancelling active recordings for: ${upgradeState.cancelAliases.join(", ")}`
      );
    let results;
    try {
      ...
      if (error?.status === "invalid-state") {
        rmSync(upgradePath);
        throw new CliError(
          "Recording state changed during upgrade checks; no companion change was made. Preview the affected bridges again."
        );
      }
      throw error;
    }
    if (
      results.some(
        (effect) =>
          upgradeState.cancelAliases.includes(effect.sshAlias) &&
          effect.activeRecordingLease < 1
      )
    ) {
      throw new CliError(
        "Companion did not confirm deletion of every affected recording."
      );
    }
  ...
  atomicWrite(
    upgradePath,
    `${JSON.stringify({ ...upgradeState, phase: "preflight-required" })}\n`
  );
  console.log(
    "Shared companion upgraded after every registered destination passed compatibility checks."
  );
  console.log(
    "Required next step: run `pi-dictation bridge preflight`; completion includes all-host authenticated health checks."
  );
}
...
function assertRemovalLayout(p) {
  ...
  for (const name of readdirSync(p.root)) {
    if (!allowedRoot.has(name))
      throw new CliError(
        `Refusing unexpected artifact '${name}' whose ownership cannot be proven.`
      );
  }
  const receipt = readJsonOwned(p.receipt, "bridge ownership receipt");
  if (receipt.product !== LABEL || typeof receipt.installId !== "string")
    throw new CliError(
      "Refusing bridge artifacts whose ownership cannot be proven."
    );
  ...
  if (pathExists(upgradePath)) {
    ...
    if (
      pending.product !== LABEL ||
      !["quiescing", "ready-to-install", "preflight-required"].includes(
        pending.phase
      ) ||
      !Array.isArray(pending.hosts)
    )
      throw new CliError("Refusing invalid bridge upgrade state.");
  }
  if (pathExists(p.plist)) {
  ...
  if (pathExists(p.runtime)) {
    inspectPath(p.runtime, "directory", 0o700, "bridge runtime directory");
    const allowed =
      /^(?:companion\.sock|companion\.log(?:\.[12])?|resource-metrics\.json|request-receipts\.json|recording-[0-9a-f-]{36}\.(?:wav|json|reserve)|request-[0-9a-f-]{36}\.json|revocation-[0-9a-f-]{36}\.json)$/i;
    for (const name of readdirSync(p.runtime)) {
      if (!allowed.test(name))
        throw new CliError(
          `Refusing unexpected runtime artifact '${name}' whose ownership cannot be proven.`
        );
...
async function uninstall(args) {
  ...
  if (
    ...
    throw new CliError(
      "Usage: pi-dictation bridge uninstall <ssh-alias>|--all [--cancel-active] [--delete-retained-wav --delete-credentials] --confirm"
    );
  }
  const p = paths();
  ...
  if (targets.length === 0 && !removeAll)
    throw new CliError("Specify an SSH alias or --all.");
  const removingLast =
    targets.length === hosts.length &&
    targets.every((alias) => hosts.some((host) => host.sshAlias === alias));
  if (targets.some((alias) => !hosts.some((host) => host.sshAlias === alias)))
    throw new CliError("Unknown host bridge; nothing was removed.");
  const pendingTargets = hosts
    .filter(
      (host) =>
        targets.includes(host.sshAlias) &&
        host.status.lifecycle === "revocation-pending"
    )
    .map((host) => host.sshAlias);
  const selected = await inspectHostEffects(
    targets.filter((alias) => !pendingTargets.includes(alias)),
    companionRequestAt
  );
  for (const sshAlias of pendingTargets)
    selected.push({
      sshAlias,
      connections: 0,
  ...
  const active = selected
    .filter((effect) => effect.activeRecordingLease > 0)
    .map((effect) => effect.sshAlias);
  console.log(`Host bridges to remove: ${targets.join(", ") || "none"}`);
  console.log(`Active recordings to cancel: ${active.join(", ") || "none"}`);
  console.log(
    `Retained WAVs to delete: ${selected.reduce((sum, effect) => sum + effect.retainedWav, 0)}`
  );
  console.log(`Credentials to delete: ${targets.length}`);
  if (active.length && !cancelActive)
    throw new CliError(
      `Active recording blocks uninstall for: ${active.join(", ")}. Rerun with --cancel-active after reviewing the deletion preview.`
    );
  const retainedWav = selected.reduce(
    (sum, effect) => sum + effect.retainedWav,
    0
  );
  if (removingLast && (!deleteAudio || !deleteCredentials)) {
    console.log(
      "Preview only. Removing the last bridge requires separate --delete-retained-wav and --delete-credentials confirmations."
    );
    return;
  }
  if (retainedWav > 0 && !deleteAudio) {
    console.log(
      "Preview only. Deleting retained WAVs requires separate --delete-retained-wav confirmation."
    );
    return;
  }
  if (!confirmed) {
    console.log(
      "Preview only. Rerun with --confirm after reviewing the exact deletion effects."
    );
    return;
  }
  ...
  for (const alias of targets)
    await revokeHost(alias, true, companionRequestAt, deletionPolicy);
  if (!removingLast) {
  ...
  console.log(
    "macOS microphone permission history may remain in Privacy & Security settings."
  );
}

function usage() {
  console.log(
    "Usage: pi-dictation bridge <build|install|preflight|health|status|list|doctor|logs|repair|upgrade|rotate|revoke|uninstall>"
  );
}
...
async function main() {
  ...
  if (command === "install" && args.length >= 1)
    return installHost(args[0], args.slice(1));
  if (command === "status" && args.length === 1) return hostStatus(args[0]);
  if (
    command === "list" &&
    (args.length === 0 || (args.length === 1 && args[0] === "--json"))
  )
    return listHosts(args[0] === "--json");
  if (
    command === "doctor" &&
    (args.length === 0 || (args.length === 1 && args[0] === "--json"))
  )
    return bridgeDoctor(args[0] === "--json");
  if (command === "logs" && args.length <= 1 && args[0] !== "--json")
    return bridgeLogs(args[0]);
  ...
  if (command === "rotate" && args.length === 1)
    return rotateHost(args[0], companionRequestAt);
  if (
    command === "revoke" &&
    (args.length === 1 || (args.length === 2 && args[1] === "--confirm"))
  )
    return revokeHost(args[0], args[1] === "--confirm", companionRequestAt);
  ...
  if (command === "remote-prepare" && args.length === 2)
    return remotePrepare(args[0], args[1]);
  if (command === "remote-credential-commit" && args.length === 3)
    return remoteCredentialCommit(args[0], args[1], args[2]);
  if (command === "remote-credential-revoke" && args.length === 1)
    return remoteCredentialRevoke(args[0]);
  if (command === "remote-recorder-removal-check" && args.length === 1)
    return remoteRecorderRemovalCheck(args[0]);
  if (command === "remote-recorder-removal-release" && args.length === 1)
    return remoteRecorderRemovalRelease(args[0]);
  if (command === "remote-listener" && args.length === 1)
    return remoteListener(args[0]);
  if (command === "remote-listener-cleanup" && args.length === 1)
    return remoteListenerCleanup(args[0]);
  if (command === "remote-health" && args.length === 1)
    return remoteHealth(args[0], healthAt);
  usage();
...
try {
  ...
  const message =
    error instanceof CliError || error instanceof BridgeHostError
      ? error.message
