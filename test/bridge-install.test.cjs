...
const cli = join(root, "dist", "bin", "pi-dictation.js");
...
`
  ...
  const child = spawn(
    process.execPath,
    [script, f.bridge, socket, busyFile, dropFile || `${busyFile}.never-drop`],
...
test("bridge install establishes the default private Unix listener through an existing SSH alias", async (t) => {
  ...
  try {
    ...
    const endpoint = JSON.parse(
      readFileSync(join(host, "endpoint.json"), "utf8")
    );
    ...
    await t.test("uses the Unix listener by default", () =>
      assert.equal(endpoint.type, "unix")
    );
    await t.test("places the listener in a per-host private directory", () =>
      assert.match(
        endpoint.path,
        /^\/srv\/pi\/\.local\/share\/pi-dictation\/bridge\/hosts\/[0-9a-f]{16}\/listener\.sock$/
      )
    );
    await t.test("requires non-interactive authentication", () =>
      assert.match(calls, /BatchMode=yes/)
    );
    await t.test(
      "preserves the SSH alias instead of copying resolved host settings",
      () => assert.match(calls, / work-pi true/)
    );
    await t.test("marks authenticated health separately", () =>
      assert.equal(state.stages.authenticatedHealth, "ready")
    );
    await t.test("prints the exact listener", () =>
      assert.match(result.stdout, /Remote listener: \/srv\/pi\//)
    );
  } finally {
...
test("each SSH alias receives independent owned artifacts", async (t) => {
  ...
  try {
    ...
    const credentials = hosts.map((id) =>
      JSON.parse(
        readFileSync(join(f.bridge, "hosts", id, "credential.json"), "utf8")
      )
    );
    await t.test("creates one directory per alias", () =>
      assert.equal(hosts.length, 2)
    );
    await t.test("uses independent credentials", () =>
      assert.notEqual(credentials[0].id, credentials[1].id)
    );
  } finally {
    rmSync(f.home, { recursive: true, force: true });
  }
...
test("confirmed revocation retries a lost response from durable local intent", async (t) => {
  ...
  try {
    ...
    const intentPersisted = existsSync(
      join(host, "credential.revocation.json")
    );
    const pendingList = run(f, ["list", "--json"]);
    const pendingLifecycle = JSON.parse(pendingList.stdout).hosts[0].status
      .lifecycle;
    const rotationWhilePending = run(f, ["rotate", "retry-revoke-pi"]);
    const retried = run(f, ["revoke", "retry-revoke-pi", "--confirm"]);

    await t.test("lost response fails the first invocation", () =>
      assert.notEqual(interrupted.status, 0)
    );
    await t.test("local revocation intent survives the lost response", () =>
      assert.equal(intentPersisted, true)
    );
    await t.test("list reports the ambiguous revocation as pending", () =>
      assert.equal(pendingLifecycle, "revocation-pending")
    );
    await t.test("rotation is refused while revocation is pending", () =>
      assert.notEqual(rotationWhilePending.status, 0)
    );
    await t.test("retry completes with the original request identity", () =>
...
test("credential rotation retries an idle rejection without restarting the companion", async (t) => {
  ...
  try {
    ...
    const stagedAfterRejection = existsSync(
      join(host, "credential.rotation.json")
    );
    rmSync(busyFile);
    ...
    await t.test("rejects rotation while the credential owns audio", () =>
      assert.notEqual(rejected.status, 0)
    );
    await t.test("preserves staged rotation after rejection", () =>
      assert.equal(stagedAfterRejection, true)
    );
    await t.test("uses a new request identity for the new attempt", () =>
      assert.notEqual(requestIds[0], requestIds[1])
    );
    await t.test("retry succeeds against the same companion", () =>
      assert.equal(retried.status, 0, retried.stderr)
    );
  } finally {
    server?.kill("SIGTERM");
...
test("remote credential commit keeps a readable credential through interruption", async (t) => {
  ...
  const prepare = (credential, stagedCredential) =>
    spawnSync(
      process.execPath,
      [
        cli,
        ...
        Buffer.from(
          JSON.stringify({
            ...endpoint,
            ...(stagedCredential ? { stagedCredential: true } : {}),
          })
        ).toString("base64"),
      ],
      {
  ...
  try {
    ...
    const interrupted = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...
          HOME: home,
          NODE_ENV: "test",
          PI_DICTATION_TEST_INTERRUPT: "after-remote-current-copy",
        },
      }
    );
    const config = JSON.parse(
      readFileSync(join(home, ".pi", "agent", "pi-dictation.json"), "utf8")
    );
    const readableDuringInterruption = existsSync(
      config.recorders.bridge.credentialFile
    );
    ...
    await t.test("interrupts after copying the current credential", () =>
      assert.notEqual(interrupted.status, 0)
    );
    await t.test("Recorder credential remains readable", () =>
      assert.equal(readableDuringInterruption, true)
    );
    await t.test("retry completes the remote commit", () =>
      assert.equal(retried.status, 0, retried.stderr)
    );
  } finally {
...
for (const [name, bind, expectedHost] of [
  ["IPv4", "127.0.0.1:43123", "127.0.0.1"],
  ["IPv6", "[::1]:43124", "::1"],
]) {
  test(`bridge install supports explicit ${name} loopback fallback`, async (t) => {
    ...
    try {
      ...
      await t.test("configures only the requested loopback host", () =>
        assert.equal(endpoint.host, expectedHost)
      );
      await t.test("shows the exact bind", () =>
        assert.match(
          result.stdout,
          new RegExp(bind.replace(/[\[\]:.]/g, "\\$&"))
        )
      );
    } finally {
      rmSync(f.home, { recursive: true, force: true });
...
test("bridge install stops when BatchMode authentication fails", async (t) => {
  ...
  try {
    ...
    await t.test("explains non-interactive authentication", () =>
      assert.match(
        result.stderr,
        /Non-interactive BatchMode authentication failed/
      )
    );
    await t.test("preserves resumable setup state", () =>
      assert.equal(state.stages.authentication, "pending")
    );
  } finally {
    rmSync(f.home, { recursive: true, force: true });
  }
...
test("tunnel configuration reuses required alias policy while disabling unrelated behavior", async (t) => {
  ...
  try {
    ...
    await t.test(
      "uses an isolated configuration with forwarding-only behavior",
      () =>
        assert.deepEqual(
          values.filter((value) =>
            [
    ...
    await t.test("preserves the one explicit remote forward", () =>
      assert.deepEqual(
        values.filter(
          (value) => value === "-R" || value.startsWith("ClearAllForwardings=")
        ),
        ["-R"]
      )
    );
    await t.test(
      "replaces only stale Unix listeners inside the private managed host directory",
      () => assert.ok(values.includes("StreamLocalBindUnlink=yes"))
    );
    await t.test("retains strict host-key verification", () =>
      assert.ok(values.includes("StrictHostKeyChecking=true"))
    );
    await t.test(
      "retains ProxyJump routing without enabling unrelated forwarding",
      () =>
        assert.match(
          values.find((value) => value.startsWith("ProxyCommand=")),
          /bastion/
        )
    );
  } finally {
...
test("repeated install refuses an unowned tunnel LaunchAgent", () => {
  ...
  try {
    ...
    if (first.status !== 0) throw new Error(first.stderr);
    const plist = require("node:fs")
      .readdirSync(join(f.home, "Library", "LaunchAgents"))
      .map((name) => join(f.home, "Library", "LaunchAgents", name))[0];
...
test("repeated install refuses an unowned setup state", () => {
  ...
  try {
    ...
    const host = join(f.bridge, "hosts", hostDirectories(f.bridge)[0]);
    writeFileSync(
      join(host, "setup.json"),
      JSON.stringify({ product: "someone-else", stages: {} }),
      { mode: 0o600 }
    );
...
test("remote prepare installs private Recorder endpoint state without a package manager", async (t) => {
  ...
  try {
    ...
    await t.test("stores a Bridge Recorder endpoint", () =>
      assert.equal(recorder.type, "bridge")
    );
    await t.test(
      "configures the Bridge profile without changing Recorder selection",
      () =>
        assert.deepEqual(runtimeConfig.recorders, {
          selected: "local",
          bridge: {
    ...
    await t.test("keeps host state private", () =>
      assert.equal(require("node:fs").lstatSync(host).mode & 0o777, 0o700)
    );
    await t.test("keeps the shared credential private", () =>
      assert.equal(
        require("node:fs").lstatSync(join(host, "credential.json")).mode &
          0o777,
        0o600
      )
    );
...
test("remote prepare rejects an oversized endpoint before Base64 decoding", () => {
  const home = mkdtempSync(
    join(tmpdir(), "pi-dictation-remote-endpoint-limit-")
  );
  try {
    const encoded = Buffer.alloc(13 * 1024, 65).toString("base64");
    const result = spawnSync(
      process.execPath,
      [cli, "bridge", "remote-prepare", "0123456789abcdef", encoded],
      {
        cwd: root,
        encoding: "utf8",
        input: "{}",
...
test("remote prepare preserves a user-selected Local Recorder while updating the Bridge profile", () => {
  ...
  try {
    const first = invoke({
      type: "unix",
      path: join(hostRoot, "listener.sock"),
    });
    if (first.status !== 0) throw new Error(first.stderr);
...
test("legacy receipt migration distinguishes unrelated configuration from Recorder changes", async (t) => {
  const scenario = () => {
    const home = mkdtempSync(
      join(tmpdir(), "pi-dictation-remote-legacy-receipt-")
    );
  ...
  await t.test("rejects a changed Bridge Recorder", () => {
    ...
    try {
      ...
      assert.match(
        result.stderr,
        /Bridge Recorder changed outside bridge setup/
      );
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
...
test("remote removal reservation permits cleanup while Local Recorder remains selected", () => {
  const home = mkdtempSync(
    join(tmpdir(), "pi-dictation-remote-reserved-revoke-")
  );
  ...
  const credential = {
    ...
    secret: Buffer.alloc(32, 20).toString("base64"),
  };
  const invoke = (command) =>
    spawnSync(
      ...
      ],
      {
        ...
        encoding: "utf8",
        input:
          command === "remote-prepare" ? JSON.stringify(credential) : undefined,
        env: { ...process.env, HOME: home },
      }
    );
  try {
...
test("remote revocation requires the Recorder selection reservation", () => {
  const home = mkdtempSync(
    join(tmpdir(), "pi-dictation-remote-selected-revoke-")
  );
  ...
  try {
    ...
    const revoked = spawnSync(
      process.execPath,
      [cli, "bridge", "remote-credential-revoke", id],
      {
        cwd: root,
        encoding: "utf8",
        env: { ...process.env, HOME: home },
...
test("remote Recorder configuration resumes an interrupted owned transaction", () => {
  ...
  try {
    ...
    const current = JSON.parse(
      readFileSync(join(configDirectory, "pi-dictation.json"), "utf8")
    );
    const tcp = { type: "tcp", host: "::1", port: 43124 };
    const next = {
      ...current,
      recorder: { ...current.recorder, endpoint: tcp },
    };
    const digest = (value) =>
      createHash("sha256").update(JSON.stringify(value)).digest("hex");
    writeFileSync(
      join(configDirectory, "pi-dictation.bridge-next.json"),
      `${JSON.stringify(next)}\n`,
      { mode: 0o600 }
    );
    writeFileSync(
      join(configDirectory, "pi-dictation.bridge-owner.json"),
      JSON.stringify({
        product: "com.yasuhito.pi-dictation.bridge",
        hostId: id,
        phase: "pending",
...
test("remote Recorder configuration recovers an owned orphaned staging file", () => {
  ...
  try {
    ...
    const previousSha256 = createHash("sha256")
    ...
    writeFileSync(
      join(configDirectory, "pi-dictation.bridge-next.json"),
      JSON.stringify({ recorder: { type: "local" } }),
      { mode: 0o600 }
    );
...
test("remote listener cleanup is a no-op for a managed TCP fallback", () => {
  ...
  writeFileSync(
    join(hostRoot, "endpoint.json"),
    JSON.stringify({
      endpoint: { type: "tcp", host: "127.0.0.1", port: 43123 },
    }),
    { mode: 0o600 }
  );
  try {
    const cleanup = spawnSync(
      process.execPath,
      [cli, "bridge", "remote-listener-cleanup", id],
      {
        cwd: root,
        encoding: "utf8",
        env: { ...process.env, HOME: home },
...
test(
  "remote listener cleanup removes only a stale managed Unix socket",
  ...
  async (t) => {
    ...
    writeFileSync(
      join(hostRoot, "endpoint.json"),
      JSON.stringify({ endpoint: { type: "unix", path: socketPath } }),
      { mode: 0o600 }
    );
    writeFileSync(
      script,
      `const net=require("node:net");net.createServer().listen(${JSON.stringify(socketPath)},()=>console.log("ready"));\n`
    );
...
test(
  "remote listener rejects an SSH TCP forward exposed by GatewayPorts",
  { skip: process.platform !== "linux" },
  async () => {
    ...
    writeFileSync(
      script,
      "const net=require('node:net');const server=net.createServer(s=>s.destroy());server.listen(0,'0.0.0.0',()=>console.log(server.address().port));\n"
    );
    const server = spawn(process.execPath, [script], {
      stdio: ["ignore", "pipe", "inherit"],
    });
...
test("tunnel supervisor establishes listener, health, and bounded safe logging", async (t) => {
  ...
  const supervisor = spawn(
    process.execPath,
    [join(root, "dist", "bin", "pi-dictation-tunnel.js"), configurationPath],
    {
      cwd: root,
      env: {
        ...process.env,
  ...
  try {
    ...
    await t.test(
      "cleans the managed remote listener before opening the tunnel",
      () => {
...
test("tunnel supervisor aggregates repeated connection failures", async (t) => {
  ...
  const supervisor = spawn(
    ...
    {
      cwd: root,
      env: { ...process.env, HOME: home, PATH: `${tools}:/usr/bin:/bin` },
      stdio: "ignore",
    }
  );
  try {
    ...
    supervisor.kill("SIGTERM");
    await once(supervisor, "exit");
    const records = readFileSync(join(home, "tunnel.log"), "utf8")
      .trim()
      .split("\n")
      .map(JSON.parse);
    assert.equal(
      records.some(
        (record) => record.code === "repeated" && record.count >= 1
      ) &&
        !/credential|secret|sshArguments|\/Users\//i.test(
...
test("remote prepare refuses to overwrite an existing unowned Pi configuration", () => {
  ...
  try {
    ...
    writeFileSync(
      join(configDirectory, "pi-dictation.json"),
...
test("bridge doctor diagnoses every layer without mutating LaunchAgents", async (t) => {
  ...
  try {
    ...
    const result = run(f, ["doctor", "--json"], {
      DOCTOR_LAUNCH_LOG: launchLog,
    });
    const report = JSON.parse(result.stdout);
    const host = report.hosts[0];
    await t.test("reports tunnel process separately", () =>
      assert.equal(host.tunnelProcess.status, "running")
    );
    await t.test("reports listener separately", () =>
      assert.equal(host.listener.status, "established")
    );
    await t.test("reports authenticated health separately", () =>
      assert.equal(host.authenticatedHealth.status, "ready")
    );
    await t.test("reports exact protocol compatibility separately", () =>
      assert.equal(host.protocolCompatibility.status, "compatible")
    );
    await t.test("reports bounded storage separately", () =>
      assert.equal(host.storage.status, "bounded")
    );
    await t.test("reports bounded connections separately", () =>
      assert.equal(host.connections.status, "bounded")
    );
    await t.test("reports Level availability separately", () =>
      assert.equal(host.levelAvailability.status, "available")
    );
    await t.test("uses only read-only launchctl print", () =>
      assert.equal(
        readFileSync(launchLog, "utf8")
          .trim()
          .split("\n")
          .every((line) => line.startsWith("print ")),
        true
      )
    );
  } finally {
...
test("remote revocation removes a partial host without changing another owned Recorder", async (t) => {
  ...
  try {
    ...
    writeFileSync(
      join(remoteRoot, "ownership.json"),
      JSON.stringify({
        product: "com.yasuhito.pi-dictation.bridge",
        hostId: partialId,
      }),
    ...
    writeFileSync(
      join(remoteRoot, "credential.json"),
      JSON.stringify({
        id: "22222222-2222-4222-8222-222222222222",
    ...
    writeFileSync(
      join(configRoot, "pi-dictation.json"),
      JSON.stringify(config),
      { mode: 0o600 }
    );
    writeFileSync(
      join(configRoot, "pi-dictation.bridge-owner.json"),
      JSON.stringify({
        product: "com.yasuhito.pi-dictation.bridge",
        hostId: ownerId,
        phase: "ready",
    ...
    await t.test("completes partial cleanup", () =>
      assert.equal(result.status, 0, result.stderr)
    );
    await t.test("removes only the partial host", () =>
      assert.equal(existsSync(remoteRoot), false)
    );
    await t.test("preserves the other owned Recorder", () =>
      assert.deepEqual(
        JSON.parse(readFileSync(join(configRoot, "pi-dictation.json"))),
        config
      )
    );
  } finally {
...
test("selected Bridge refusal happens before credential revocation", async (t) => {
  ...
  try {
    ...
    server = await startCredentialServer(f, effectsFile);
    const host = join(f.bridge, "hosts", hostDirectories(f.bridge)[0]);
    const result = run(f, ["revoke", "selected-pi", "--confirm"], {
      SSH_SELECTED_BRIDGE: "1",
    });
    await t.test("refuses with Recorder selection guidance", () => {
      assert.match(
        result.stderr,
        /Select Local recording with \/dictate-config/
      );
    });
...
test("scoped uninstall removes only the selected host bridge", async (t) => {
  ...
  try {
    ...
    writeFileSync(
      join(runtime, "request-receipts.json"),
      JSON.stringify({ schemaVersion: 4, receipts: [] }),
      { mode: 0o600 }
    );
    const result = run(f, ["uninstall", "remove-pi", "--confirm"]);
    const listed = run(f, ["list", "--json"]);
    const remaining = JSON.parse(listed.stdout).hosts.map(
      (host) => host.sshAlias
    );
    await t.test("completes the scoped uninstall", () =>
      assert.equal(result.status, 0, result.stderr)
    );
    await t.test("preserves the other host", () =>
      assert.deepEqual(remaining, ["keep-pi"])
    );
    await t.test("preserves the shared companion receipt", () =>
      assert.equal(existsSync(join(f.bridge, "ownership.json")), true)
    );
  } finally {
    server?.kill("SIGTERM");
    if (server) await once(server, "exit").catch(() => {});
...
test("active recording blocks uninstall without explicit cancellation", async (t) => {
  ...
  try {
    ...
    await t.test("names the affected bridge", () =>
      assert.match(result.stderr, /active-pi/)
    );
    await t.test("requires explicit cancellation", () =>
      assert.match(result.stderr, /--cancel-active/)
    );
    await t.test("preserves the host bridge", () =>
      assert.equal(hostDirectories(f.bridge).length, 1)
    );
  } finally {
    server?.kill("SIGTERM");
    if (server) await once(server, "exit").catch(() => {});
...
test("uninstall reconciles a partial remote-cleanup failure", async (t) => {
  ...
  try {
    ...
    server = await startCredentialServer(f, effectsFile);
    const interrupted = run(f, ["uninstall", "partial-pi", "--confirm"], {
      SSH_REMOTE_REVOKE_FAIL: "1",
    });
    const pending = run(f, ["list", "--json"]);
    const lifecycle = JSON.parse(pending.stdout).hosts.find(
      (host) => host.sshAlias === "partial-pi"
    ).status.lifecycle;
    const retried = run(f, ["uninstall", "partial-pi", "--confirm"]);
    await t.test("reports the interrupted cleanup", () =>
      assert.notEqual(interrupted.status, 0)
    );
    await t.test("records a pending lifecycle", () =>
      assert.equal(lifecycle, "revocation-pending")
    );
    await t.test("finishes from durable intent", () =>
      assert.equal(retried.status, 0, retried.stderr)
    );
  } finally {
    server?.kill("SIGTERM");
...
function completeCompanionFixture(f) {
  ...
  executable(
    join(app, "Contents", "MacOS", "PiDictationBridge"),
    "#!/bin/sh\nexit 0\n"
  );
  executable(
    join(app, "Contents", "MacOS", "PiDictationDurationWatchdog"),
    "#!/bin/sh\nexit 0\n"
  );
  writeFileSync(
    join(app, "Contents", "Resources", "ownership.json"),
    JSON.stringify({ product: "com.yasuhito.pi-dictation.bridge", installId }),
    { mode: 0o600 }
  );
  writeFileSync(
    join(
  ...
  writeFileSync(
    join(f.bridge, "preflight.json"),
    JSON.stringify({
      product: "com.yasuhito.pi-dictation.bridge",
      installId,
      executableSha256: createHash("sha256")
        .update(
          readFileSync(join(app, "Contents", "MacOS", "PiDictationBridge"))
        )
        .digest("hex"),
    }),
    { mode: 0o600 }
...
test("upgrade checks every destination before modifying the shared companion", async (t) => {
  ...
  try {
    ...
    const before = createHash("sha256")
      .update(readFileSync(join(app, "Contents", "MacOS", "PiDictationBridge")))
      .digest("hex");
    const result = run(f, ["upgrade"], { SSH_AUTH_FAIL: "1" });
    const after = createHash("sha256")
      .update(readFileSync(join(app, "Contents", "MacOS", "PiDictationBridge")))
      .digest("hex");
    await t.test("stops when a destination is unreachable", () =>
      assert.notEqual(result.status, 0)
    );
    await t.test("does not change the shared companion", () =>
      assert.equal(after, before)
    );
    await t.test("preserves real-audio readiness", () =>
      assert.equal(existsSync(join(f.bridge, "preflight.json")), true)
    );
  } finally {
    rmSync(f.home, { recursive: true, force: true });
  }
...
test("active recording blocks upgrade and names the affected bridge", async (t) => {
  ...
  try {
    ...
    await t.test("blocks before companion replacement", () =>
      assert.notEqual(result.status, 0)
    );
    await t.test("names the active bridge", () =>
      assert.match(result.stderr, /upgrade-active-pi/)
    );
    await t.test("requires explicit confirmed cancellation", () =>
      assert.match(result.stderr, /--cancel-active --confirm/)
    );
    await t.test("preserves real-audio readiness", () =>
      assert.equal(existsSync(join(f.bridge, "preflight.json")), true)
    );
...
test("uninstall refuses an unexpected artifact inside an owned host directory", async (t) => {
  ...
  try {
    ...
    await t.test("refuses the artifact", () =>
      assert.match(result.stderr, /unexpected host artifact/)
    );
    await t.test("preserves the artifact", () =>
      assert.equal(readFileSync(artifact, "utf8"), "external\n")
    );
  } finally {
    rmSync(f.home, { recursive: true, force: true });
  }
