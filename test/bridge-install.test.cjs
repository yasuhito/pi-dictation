...
case "$*" in
  ...
  *" remote-info") if [ "$SSH_WRONG_VERSION" = 1 ]; then version=9.9.9; else version=0.6.0; fi; printf '{"packageVersion":"%s","protocolVersion":1,"home":"/srv/pi"}\\n' "$version"; exit 0 ;;
  *" remote-prepare "*) cat >/dev/null; printf '{"configured":true}\\n'; exit 0 ;;
  *" remote-listener "*) printf '{"listener":"established"}\\n'; exit 0 ;;
  *" remote-health "*) printf '{"protocolVersion":1,"authenticatedHealth":"ok"}\\n'; exit 0 ;;
esac
exit 2
`);
