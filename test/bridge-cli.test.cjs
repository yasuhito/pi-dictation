...
const server = net.createServer({ allowHalfOpen: true }, (socket) => {
  ...
  socket.write(frame({ type: "challenge", version: mode === "wrong-version" ? 2 : 1, challenge: challenge.toString("base64") }));
  ...
  socket.on("data", (chunk) => {
    ...
    const expected = tag(["request", 1, challenge, credential.id, request.requestId, "health", payload]);
    ...
    const responseTag = mode === "bad-response-hmac"
      ? Buffer.alloc(32)
      : tag(["response", 1, challenge, credential.id, request.requestId, "health:ok", responsePayload]);
    const response = frame({ type: "response", version: 1, requestId: request.requestId, status: "ok", payload: responsePayload.toString("base64"), hmac: responseTag.toString("hex") });
...
test("bridge health authenticates an exact-version request and response over the private Unix socket", async (t) => {
  ...
  try {
    ...
    await t.test("reports exact protocol compatibility", () => {
      assert.match(result.stdout, /Protocol: ok \(exact version 1\)/);
    });
...
test("bridge health rejects any other protocol version", () => {
  return (async () => {
    ...
    try {
      ...
      const result = runBridge(home, ["health"]);
      assert.match(result.stderr, /requires exact version 1/);
    } finally {
      server?.kill();
      rmSync(home, { recursive: true, force: true });
    }
