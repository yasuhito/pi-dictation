...
test("the Bridge Recorder routes all exchanges through the shared protocol seam", async (t) => {
  ...
  const streams = [];
  try {
    const sharedProtocol = await import(
      join(root, "dist", "lib", "bridge-protocol.js")
    );
    const { checkBridgeRecorder, createBridgeRecorder } = await jiti.import(
      join(root, "dist", "extensions", "bridge-recorder.js")
    );
    const protocol = {
      request(options) {
    ...
    await t.test("routes audio and Level streams through withStream", () => {
      assert.deepEqual(
        [...new Map(streams.map((value) => [value[0], value])).values()].sort(),
        [
          ["fetch", "binary"],
          ["subscribe-levels", "authenticated-frames"],
        ]
      );
    });
  } finally {
...
test("the Bridge health check reports an authenticated available input", async () => {
  const instance = await harness();
  try {
    const { checkBridgeRecorder } = await jiti.import(
      join(root, "dist", "extensions", "bridge-recorder.js")
    );
...
test("the Bridge health check retries an ambiguous transport outcome", async (t) => {
  ...
  try {
    ...
    const protocol = {
      request(options) {
        deadlines.push(
          options.timing.challenge.at,
          options.timing.requestWrite.at,
          options.timing.response.at
        );
        return sharedProtocol.request(options);
      },
    };
    const available = await checkBridgeRecorder(
      instance.config,
      2000,
      protocol
    );
    const healthEvents = instance
      .events()
      .filter((event) => event === "health");
    const requestIds = instance
      .events()
    ...
    await t.test(
      "keeps one caller-owned bound across authentication and retries",
      () => {
...
test("Bridge fetch and SHA-256 buffers remain fixed as WAV length grows", async (t) => {
  const measure = async (mode) => {
    ...
    try {
      const recording = await instance.recorder.start({
  ...
  await t.test("bounds socket buffering independently of WAV length", () =>
    assert.equal(Math.max(short.socket, long.socket) <= 128 * 1024 + 4, true)
  );
  await t.test("bounds fetch chunks independently of WAV length", () =>
    assert.equal(Math.max(short.fetch, long.fetch) <= 64 * 1024, true)
  );
  await t.test("bounds SHA-256 chunks independently of WAV length", () =>
    assert.equal(Math.max(short.sha256, long.sha256) <= 64 * 1024, true)
  );
});

test("the Recorder starts with an install or rotation credential containing creation metadata", async () => {
  const instance = await harness("valid", {
...
test("the Bridge Recorder refuses a credential whose identity is not canonical", async (t) => {
  ...
  try {
    ...
    const { createBridgeRecorder } = await jiti.import(
      join(root, "dist", "extensions", "bridge-recorder.js")
    );
...
test("the Bridge Recorder preserves the safe audio error for trailing ordinary response bytes", async (t) => {
  ...
  try {
    const error = await instance.recorder
      .start(instance.startOptions)
      .catch((value) => value);
    ...
    await t.test("returns the prior safe message", () => {
      assert.equal(
        error.message,
        "The recorder did not produce a complete PCM16 mono WAV."
      );
    });
  } finally {
    await instance.cleanup();
...
test("the Bridge Recorder exposes storage reservation exhaustion safely", async () => {
  ...
  try {
    await assert.rejects(instance.recorder.start(instance.startOptions), {
      code: "recorder-storage-full",
    });
  } finally {
    await instance.cleanup();
  }
...
test("the Recorder transports authenticated Bridge recording Level observations", async () => {
  ...
  try {
    const recording = await instance.recorder.start({
      ...instance.startOptions,
      onLevel: (value) => observations.push(value),
    });
    ...
    await recording.cancel();
    assert.deepEqual(
      observations.find(({ type }) => type === "observation"),
      {
        type: "observation",
        sequence: 0,
        capturedAtMs: 0,
...
test("the Bridge Level subscription replays the fixed thirty-second history", async (t) => {
  ...
  try {
    const recording = await instance.recorder.start({
      ...instance.startOptions,
      onLevel: (event) => observations.push(event),
    });
    const deadline = Date.now() + 3000;
    while (
      observations.filter(({ type }) => type === "observation").length < 600 &&
      Date.now() < deadline
    ) {
      await new Promise((resolveWait) => setTimeout(resolveWait, 20));
    }
...
test("the Bridge Level subscription accepts in-window out-of-order observations", async () => {
  ...
  try {
    ...
    const deadline = Date.now() + 3000;
    while (
      observations.filter(({ type }) => type === "observation").length < 2 &&
      Date.now() < deadline
    ) {
      await new Promise((resolveWait) => setTimeout(resolveWait, 20));
    }
...
test("the Bridge Level subscription rejects a malformed authenticated frame through the shared seam", async () => {
  ...
  try {
    const recording = await instance.recorder.start({
      ...instance.startOptions,
      onLevel: (event) => events.push(event),
    });
    const deadline = Date.now() + 3000;
    while (
      !events.some(
        ({ type, state }) => type === "transport" && state === "unavailable"
      ) &&
      Date.now() < deadline
    ) {
      await new Promise((resolveWait) => setTimeout(resolveWait, 20));
...
test("the Bridge rejects conflicting duplicate Level observations", async (t) => {
  ...
  try {
    ...
    await t.test("does not deliver the conflicting duplicate", () => {
      assert.equal(
        events.filter(
          ({ type, sequence }) => type === "observation" && sequence === 0
        ).length,
        1
      );
    });
...
test("the Bridge Level subscription reports observations lost beyond replay", async () => {
  ...
  try {
    ...
    const deadline = Date.now() + 3000;
    while (
      !observations.some(({ type }) => type === "gap") &&
      Date.now() < deadline
    ) {
      await new Promise((resolveWait) => setTimeout(resolveWait, 20));
    }
    await recording.cancel();
    assert.deepEqual(
      observations.find(({ type }) => type === "gap"),
      { type: "gap", fromSequence: 0, toSequence: 4 }
    );
  } finally {
    await instance.cleanup();
  }
...
test("a terminal Level frame ends the subscription loop", async (t) => {
  ...
  try {
    ...
    const subscriptionCount = instance
      .events()
      .filter((event) => event === "subscribe-levels").length;
    await recording.cancel();
...
test("the Bridge retries a failed Level connection with bounded backoff only while recording", async (t) => {
  ...
  try {
    ...
    const subscriptionsWhileRecording = instance
      .events()
      .filter((event) => event === "subscribe-levels").length;
    await recording.cancel();
    await new Promise((resolveWait) => setTimeout(resolveWait, 250));
    const subscriptionsAfterCancel = instance
      .events()
      .filter((event) => event === "subscribe-levels").length;
...
test("measurement unavailability remains explicit on the Recorder boundary", async () => {
  ...
  try {
    ...
    const deadline = Date.now() + 3000;
    while (
      !observations.some(({ type }) => type === "unavailable") &&
      Date.now() < deadline
    ) {
      await new Promise((resolveWait) => setTimeout(resolveWait, 20));
    }
  ...
  test(`the Bridge recording adapter rejects ${mode} without committing audio`, async (t) => {
    ...
    try {
      const error = await instance.recorder
        ...
        .catch((value) => value);
      await t.test("returns a stable safe Recorder classification", () =>
        assert.equal(error.code, code)
      );
      await t.test("leaves the destination absent", () =>
        assert.equal(existsSync(instance.startOptions.destination), false)
      );
      await t.test("deletes the private partial file", () =>
        assert.equal(
          readdirSync(resolve(instance.startOptions.destination, "..")).some(
            (name) => name.includes(".partial-")
          ),
          false
        )
      );
...
test("the Bridge recording adapter accepts result metadata when an ambiguous start retry finds a completed lease", async () => {
  ...
  try {
    ...
    await recording.cancel();
    assert.equal(
      instance.events().filter((event) => event === "start").length,
      2
    );
  } finally {
    await instance.cleanup();
...
test("each Bridge control operation keeps one deadline across transport retries", async (t) => {
  ...
  for (const operation of operations) {
    await t.test(policy[operation], () => {
      ...
      assert.equal(
        elapsed[operation] >= minimum && elapsed[operation] < maximum,
        true
      );
    });
  }
  ...
  test(`the Bridge Recorder does not retry authenticated start status ${status}`, async (t) => {
    ...
    try {
      const error = await instance.recorder
        ...
        .catch((value) => value);
      await t.test(
        "maps the authenticated status to the existing safe outcome",
        () => {
          assert.equal(
            error.code,
            status === "busy" ? "recorder-busy" : "recording-failed"
          );
        }
      );
...
test("a deterministic local fetch write failure does not enter transport recovery", async (t) => {
  const instance = await harness();
  instance.startOptions.destination = join(
    resolve(instance.startOptions.destination, ".."),
    "missing",
    "recording.wav"
  );
  try {
    ...
    const elapsed = Date.now() - startedAt;
    await t.test(
      "returns the deterministic recording failure classification",
      () => {
        assert.equal(error.code, "recording-failed");
      }
    );
    ...
    await t.test("does not retry the successful remote fetch", () => {
      assert.equal(
        instance.events().filter((event) => event === "fetch").length,
        1
      );
    });
  } finally {
...
test("the Bridge fetch deadline resets when audio transfer makes progress", async (t) => {
  ...
  try {
    ...
    await recording.stop();
    await t.test(
      "allows total transfer time to exceed one no-progress interval",
      () => {
...
test("two authenticated Bridge clients share one companion without sharing a Recording lease", async (t) => {
  ...
  try {
    ...
    const recorders = credentialFiles.map((credentialFile) =>
      createRecorder({
        type: "bridge",
        endpoint: { type: "unix", path: socket },
        credentialFile,
    ...
    const secondError = await recorders[1]
      .start(options("second.wav"))
      .catch((error) => error);
    await first.stop();

    await t.test(
      "the competing client receives only the safe busy classification",
      () => {
...
test("Bridge cancellation after acknowledgement begins is a no-op", async (t) => {
  ...
  try {
    ...
    while (!instance.events().includes("acknowledge"))
      await new Promise((resolveWait) => setTimeout(resolveWait, 10));
    await recording.cancel();
    await stopping;
    await t.test("preserves the acknowledged destination", () =>
      assert.equal(existsSync(instance.startOptions.destination), true)
    );
    await t.test("does not send a late remote cancellation", () =>
      assert.equal(instance.events().includes("cancel"), false)
    );
  } finally {
    await instance.cleanup();
  }
...
test("Bridge cancellation retries remotely when acknowledgement fails", async (t) => {
  ...
  try {
    ...
    await t.test("does not report the failed stop as successful", () =>
      assert.equal(stopError.code, "recording-failed")
    );
    await t.test("sends authenticated cancellation after the failure", () =>
      assert.equal(instance.events().includes("cancel"), true)
    );
    await t.test("leaves no transcribable destination", () =>
      assert.equal(existsSync(instance.startOptions.destination), false)
    );
  } finally {
    await instance.cleanup();
...
test("repeated concurrent Bridge lifecycle calls are idempotent", async (t) => {
  ...
  try {
    const stoppingRecording = await stoppingInstance.recorder.start(
      stoppingInstance.startOptions
    );
    await Promise.all([
      stoppingRecording.stop(),
      stoppingRecording.stop(),
      stoppingRecording.stop(),
    ]);
    const cancellingRecording = await cancellingInstance.recorder.start(
      cancellingInstance.startOptions
    );
    await Promise.all([
      cancellingRecording.cancel(),
      cancellingRecording.cancel(),
      cancellingRecording.cancel(),
    ]);
    await t.test("finalizes only once", () =>
      assert.equal(
        stoppingInstance.events().filter((event) => event === "stop").length,
        1
      )
    );
    await t.test("cancels only once", () =>
      assert.equal(
        cancellingInstance.events().filter((event) => event === "cancel")
          .length,
        1
      )
    );
...
test("Bridge cancellation interrupts finalization immediately", async (t) => {
  ...
  try {
    const recording = await instance.recorder.start(instance.startOptions);
    const stopping = recording.stop().catch((error) => error);
    while (!instance.events().includes("status"))
      await new Promise((resolveWait) => setTimeout(resolveWait, 10));
    const startedAt = Date.now();
    await recording.cancel();
    const stopError = await stopping;
    await t.test("does not wait for the finalization deadline", () =>
      assert.equal(Date.now() - startedAt < 1000, true)
    );
    await t.test("classifies the interrupted stop as cancelled", () =>
      assert.equal(stopError.code, "cancelled")
    );
    await t.test("leaves no Pi partial file", () =>
      assert.equal(
        readdirSync(resolve(instance.startOptions.destination, "..")).some(
          (name) => name.includes(".partial-")
        ),
...
test("Bridge cancellation interrupts a stalled WAV transfer", async (t) => {
  ...
  try {
    const recording = await instance.recorder.start(instance.startOptions);
    const stopping = recording.stop().catch((error) => error);
    while (!instance.events().includes("fetch"))
      await new Promise((resolveWait) => setTimeout(resolveWait, 10));
    const startedAt = Date.now();
    ...
    await cancelling;
    await t.test("does not wait for the transfer deadline", () =>
      assert.equal(stopElapsed < 1000, true)
    );
    await t.test("classifies the interrupted transfer as cancelled", () =>
      assert.equal(stopError.code, "cancelled")
    );
    await t.test("leaves no transcribable destination", () =>
      assert.equal(existsSync(instance.startOptions.destination), false)
...
test("Bridge cancellation interrupts WAV validation", async (t) => {
  ...
  try {
    ...
    while (Date.now() < deadline) {
      partial = readdirSync(directory).find((name) =>
        name.includes(".partial-")
      );
      if (
        partial &&
        statSync(join(directory, partial)).size >= 30 * 1024 * 1024
      )
        break;
    ...
    await recording.cancel();
    const stopError = await stopping;
    await t.test("classifies validation interruption as cancelled", () =>
      assert.equal(stopError.code, "cancelled")
    );
    await t.test("removes the validation partial", () =>
      assert.equal(
...
test("unconfirmed Bridge cancellation is bounded and safe", async (t) => {
  ...
  try {
    ...
    const elapsed = Date.now() - startedAt;
    await t.test("reports the owner-liveness risk", () =>
      assert.equal(error.code, "cancellation-unconfirmed")
    );
    await t.test("returns at the five-second bound", () =>
...
for (const [mode, label] of [
  ["companion-duration-disabled", "Pi"],
  ["mac-duration-early", "Mac companion"],
]) {
...
test("Bridge WAV validation distinguishes digital silence from quiet input", async (t) => {
  ...
  try {
    const silentError = await silent.recorder
      .start(silent.startOptions)
      .then((recording) => recording.stop())
      .catch((error) => error);
    const quietRecording = await quiet.recorder.start(quiet.startOptions);
    await quietRecording.stop();
    await t.test("rejects an all-zero completed WAV", () =>
      assert.equal(silentError.code, "invalid-audio")
    );
    await t.test("accepts a quiet non-zero completed WAV", () =>
      assert.equal(existsSync(quiet.startOptions.destination), true)
    );
  } finally {
...
test("Pi measures the first owner-liveness proof from the Recording lease request", async () => {
  ...
  try {
    ...
    while (firstStatusAt === undefined && Date.now() < deadline) {
      firstStatusAt = instance
        .events()
        .filter((event) => event.startsWith("status-at:"))
        .map((event) => Number(event.slice("status-at:".length)))[0];
      if (firstStatusAt === undefined)
        await new Promise((resolveWait) => setTimeout(resolveWait, 20));
    }
    await recording.cancel();
    assert.equal(
      firstStatusAt - requestedAt >= 4500 &&
        firstStatusAt - requestedAt <= 5250,
      true
    );
  } finally {
    await instance.cleanup();
...
test("Pi proves owner liveness with fresh authenticated status requests every five seconds", async (t) => {
  ...
  try {
    ...
    await t.test("starts proofs on the fixed five-second cadence", () => {
      assert.equal(
        statusTimes[1] - statusTimes[0] >= 4500 &&
          statusTimes[1] - statusTimes[0] <= 5500,
        true
      );
    });
  } finally {
...
test("a new recording fails promptly during reconnect and succeeds after authenticated service returns", async (t) => {
  ...
  try {
    ...
    const startedAt = Date.now();
    const unavailable = await instance.recorder
      .start(instance.startOptions)
      .catch((value) => value);
    ...
    replacement = fork(
      ...
      [
        instance.socket,
        Buffer.from(JSON.stringify(instance.credential)).toString("base64"),
        "valid",
        instance.eventFile,
      ],
      { stdio: ["ignore", "ignore", "ignore", "ipc"] }
    ...
    await t.test(
      "starts a new Recording lease after authenticated service returns",
      () => {
        assert.equal(
          instance.events().filter((event) => event === "start").length >= 1,
          true
        );
      }
    );
...
test("the Bridge recording adapter refuses a Unix socket outside a private directory", async () => {
  ...
  try {
    require("node:fs").chmodSync(
      resolve(instance.startOptions.destination, "../socket"),
      0o755
    );
    await assert.rejects(
      instance.recorder.start(instance.startOptions),
      (error) => error.code === "recording-failed"
    );
  } finally {
    await instance.cleanup();
  }
