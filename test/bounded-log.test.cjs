...
const modulePromise = import("../dist/bin/bounded-log.js");
...
test("bounded bridge logs rotate, aggregate, and redact", async (t) => {
  ...
  try {
    ...
    for (let index = 0; index < 60000; index += 1) {
      logger.event(index % 2 ? "load-a" : "load-b", {
        stage: "probe",
        retry: index,
      });
    }
    ...
    await t.test("keeps exactly three generations", () =>
      assert.equal(files.length, 3)
    );
    await t.test("keeps every generation at or below one MiB", () =>
      assert.equal(
        files.every((file) => statSync(file).size <= 1024 * 1024),
        true
      )
    );
    await t.test("redacts rejected field values", () =>
      assert.equal(contents.includes(secret), false)
    );
    await t.test("aggregates consecutive repeated events", () =>
