...
test("the Bridge protocol binds the request HMAC to the complete transcript", async () => {
  ...
  const expected = tag(harness.credential.secret, [
    ...
    3,
    Buffer.alloc(32, 23),
    harness.credential.id,
    sent.requestId,
    sent.operation,
    harness.state.payloadBytes,
  ]).toString("hex");
    ...
    assert.deepEqual(
      {
        kind: error.kind,
        stage: error.stage,
        connects: harness.state.connects,
      },
...
test("the Bridge protocol rejects a non-object payload before connecting", async () => {
  ...
  assert.deepEqual(
    { kind: error.kind, connects: harness.state.connects },
    { kind: "malformed", connects: 0 }
  );
});
...
test("the Bridge protocol rejects sparse nested arrays before connecting", async () => {
  ...
  const harness = createRequestHarness();
  const error = await harness
    .request({ payload: { values: Array(1) } })
    .catch((value) => value);
  assert.deepEqual(
    { kind: error.kind, connects: harness.state.connects },
    { kind: "malformed", connects: 0 }
...
test("the Bridge protocol classifies connection failures", async () => {
  ...
  assert.deepEqual(failureShape(error), {
    name: "BridgeProtocolFailure",
    kind: "transport",
    stage: "connect",
  });
});
...
test("the Bridge protocol requires a canonical lowercase request identity", async () => {
  ...
  const harness = createRequestHarness();
  const error = await harness
    .request({ requestId: randomUUID().toUpperCase() })
    .catch((value) => value);
  assert.deepEqual(
    { kind: error.kind, connects: harness.state.connects },
    { kind: "malformed", connects: 0 }
...
test("the Bridge protocol requires a canonical credential identity", async () => {
  ...
  const credential = {
    ...harness.credential,
    id: harness.credential.id.toUpperCase(),
  };
...
test("the Bridge protocol publishes its exact version", async () => {
  const { protocolVersion } = await import("../dist/lib/bridge-protocol.js");
  assert.equal(protocolVersion, 3);
});

test("the Bridge protocol publishes one canonical identity form", async (t) => {
  const { isCanonicalIdentity } =
    await import("../dist/lib/bridge-protocol.js");
  ...
  await t.test("rejects an unsupported version digit", () => {
    assert.equal(
      isCanonicalIdentity("11111111-1111-6111-8111-111111111111"),
      false
    );
  });
  await t.test("rejects an unsupported variant digit", () => {
    assert.equal(
      isCanonicalIdentity("11111111-1111-4111-c111-111111111111"),
      false
    );
  });
...
test("the Bridge protocol rejects an invalid TCP port as malformed input", async () => {
  ...
  const harness = createRequestHarness();
  const error = await harness
    .request({ endpoint: { type: "tcp", host: "127.0.0.1", port: 0 } })
    .catch((value) => value);
  assert.deepEqual(failureShape(error), {
    name: "BridgeProtocolFailure",
    kind: "malformed",
...
test("an expired absolute challenge deadline cannot write a request", async () => {
  ...
  const error = await harness
    ...
    .catch((value) => value);
  assert.deepEqual(
    { kind: error.kind, stage: error.stage, wrote: harness.state.request },
  ...
  const error = await harness
    .request({
      timing: {
        connect: { kind: "no-progress", timeoutMs: 100 },
        challenge: { kind: "no-progress", timeoutMs: 100 },
        requestWrite: { kind: "absolute", at: 0 },
        response: { kind: "no-progress", timeoutMs: 100 },
      },
    })
    .catch((value) => value);
  ...
  const error = await harness
    .request({
      timing: {
        ...
        challenge: { kind: "no-progress", timeoutMs: 100 },
        requestWrite: { kind: "no-progress", timeoutMs: 100 },
        response: { kind: "phase", timeoutMs: 150 },
      },
    })
    .catch((value) => value);
...
test("the Bridge protocol classifies a challenge deadline", async () => {
  ...
  const error = await harness
    .request({
      timing: {
        connect: { kind: "no-progress", timeoutMs: 100 },
        challenge: { kind: "absolute", at: Date.now() + 10 },
        requestWrite: { kind: "no-progress", timeoutMs: 100 },
        response: { kind: "no-progress", timeoutMs: 100 },
      },
    })
...
for (const [label, challengeBytes] of [
  [
    "an oversized challenge frame",
    (() => {
      const value = Buffer.alloc(4);
      value.writeUInt32BE(64 * 1024 + 1);
      return value;
    })(),
  ],
  [
    "invalid challenge UTF-8",
    (() => {
  ...
  [
    "recursive duplicate challenge keys",
    (() => {
      const body = Buffer.from(
        '{"type":"challenge","challenge":"AA==","nested":{"x":1,"x":2}}'
      );
      const header = Buffer.alloc(4);
  ...
  [
    "a noncanonical challenge encoding",
    (() => {
      const body = Buffer.from(
        JSON.stringify({ type: "challenge", challenge: "AB==" })
      );
      const header = Buffer.alloc(4);
      header.writeUInt32BE(body.length);
      return Buffer.concat([header, body]);
  ...
  test(`the Bridge protocol rejects ${label}`, async () => {
    const { createRequestHarness } = await loadFactory();
    const error = await createRequestHarness({ challengeBytes })
      .request()
      .catch((value) => value);
...
test("the Bridge protocol rejects bytes coalesced after the challenge frame", async () => {
  ...
  const challenge = frame({
    type: "challenge",
    challenge: Buffer.alloc(32, 23).toString("base64"),
  });
  const error = await createRequestHarness({
    challengeBytes: Buffer.concat([challenge, Buffer.from([0])]),
  })
    .request()
    .catch((value) => value);
  assert.deepEqual(failureShape(error), {
...
test("the Bridge protocol distinguishes early challenge EOF as transport failure", async () => {
  const { createRequestHarness } = await loadFactory();
  const error = await createRequestHarness({
    challengeBytes: Buffer.from([0, 0]),
    challengeEof: true,
  })
    .request()
    .catch((value) => value);
...
test("the production Unix transport classifies EOF from a real socket", async () => {
  ...
  try {
    ...
    const harness = createRequestHarness();
    const error = await harness
      .request({ endpoint: { type: "unix", path: socketPath } })
      .catch((value) => value);
    assert.deepEqual(failureShape(error), {
      name: "BridgeProtocolFailure",
      kind: "transport",
...
test("the Bridge protocol rejects a noncanonical response Base64 encoding", async () => {
  const { createRequestHarness } = await loadFactory();
  const error = await createRequestHarness({ responsePayloadEncoding: "e30" })
    .request()
    .catch((value) => value);
  assert.equal(error.kind, "malformed");
});
...
test("the Bridge protocol rejects noncanonical response hexadecimal authentication", async () => {
  const { createRequestHarness } = await loadFactory();
  const error = await createRequestHarness({ responseHmac: "AA".repeat(32) })
    .request()
    .catch((value) => value);
...
test("the Bridge protocol rejects an altered response HMAC", async () => {
  const { createRequestHarness } = await loadFactory();
  const error = await createRequestHarness({ responseHmac: "00".repeat(32) })
    .request()
    .catch((value) => value);
  assert.equal(error.kind, "authentication");
});
...
test("the Bridge protocol rejects a changed response request identity", async () => {
  const { createRequestHarness } = await loadFactory();
  const error = await createRequestHarness({ responseRequestId: randomUUID() })
    .request()
    .catch((value) => value);
  assert.equal(error.kind, "malformed");
});
...
test("the Bridge protocol rejects an unknown authenticated status envelope", async () => {
  const { createRequestHarness } = await loadFactory();
  const error = await createRequestHarness({ status: "surprise" })
    .request()
    .catch((value) => value);
  assert.equal(error.kind, "malformed");
});
...
test("withStream returns an authenticated non-success response", async (t) => {
  ...
  const harness = createRequestHarness({
    status: "not-found",
    responsePayload: { retained: false },
  });
  let invoked = false;
  const response = await harness.withStream("binary", () => {
    invoked = true;
  });
...
test("a binary stream exposes authenticated metadata before bounded bytes", async (t) => {
  const { createRequestHarness } = await loadFactory();
  const harness = createRequestHarness({
    responsePayload: { declared: 3 },
    streamBytes: Buffer.from("abc"),
  });
  const chunks = [];
  let exposedMetadata;
...
test("binary reads remain bounded when the declared transfer is large", async () => {
  const { createRequestHarness } = await loadFactory();
  const harness = createRequestHarness({
    streamBytes: Buffer.alloc(64 * 1024 + 1),
  });
  let largest = 0;
  await harness.withStream("binary", async ({ bytes }) => {
    for await (const chunk of bytes.readExactly(64 * 1024 + 1))
...
test("a binary stream rejects premature EOF", async () => {
  ...
  const error = await harness
    .withStream("binary", async ({ bytes }) => {
      for await (const _chunk of bytes.readExactly(3)) {
        /* consume */
      }
    })
    .catch((value) => value);
...
test("a binary stream rejects bytes trailing its exact read", async () => {
  ...
  const error = await harness
    .withStream("binary", async ({ bytes }) => {
      for await (const _chunk of bytes.readExactly(2)) {
        /* consume */
      }
    })
    .catch((value) => value);
...
test("an authenticated-frame stream exposes strictly parsed generic payloads", async () => {
  ...
  const harness = createRequestHarness({
    framePayloads: [
      { arbitrary: [true, null] },
      "terminal meaning stays outside",
    ],
  });
  const observed = [];
...
test("an authenticated-frame stream survives byte-by-byte fragmentation", async () => {
  const { createRequestHarness } = await loadFactory();
  const harness = createRequestHarness({
    framePayloads: [{ sequence: "payload" }],
    fragmentResponse: true,
  });
  let observed;
  await harness.withStream("authenticated-frames", async ({ frames }) => {
...
test("an authenticated-frame stream rejects recursive duplicate payload keys", async () => {
  ...
  const harness = createRequestHarness({
    framePayloads: [{}],
    frameOverrideSequence: 0,
    frameOverrides: { payloadBytes: Buffer.from('{"outer":{"x":1,"x":2}}') },
  });
  const error = await harness
    .withStream("authenticated-frames", async ({ frames }) => {
...
test("an authenticated-frame stream rejects altered authentication", async () => {
  ...
  const harness = createRequestHarness({
    framePayloads: [{}],
    frameOverrideSequence: 0,
    frameOverrides: { hmac: "00".repeat(32) },
  });
  const error = await harness
    .withStream("authenticated-frames", async ({ frames }) => {
      for await (const _payload of frames) break;
...
test("a stream no-progress deadline interrupts its consumer", async () => {
  ...
  const error = await harness
    .withStream(
      "binary",
      async ({ bytes }) => {
        for await (const _chunk of bytes.readExactly(1)) {
          /* consume */
        }
      },
      {
        timing: streamTiming({
          stream: { kind: "no-progress", timeoutMs: 10 },
        }),
      }
    )
    .catch((value) => value);
...
test("stream progress resets a no-progress deadline", async () => {
  ...
  await harness.withStream(
    "binary",
    async ({ bytes }) => {
      for await (const chunk of bytes.readExactly(3))
...
test("withStream preserves a consumer exception after cleaning up", async (t) => {
  ...
  const error = await harness
    .withStream("authenticated-frames", () => {
      throw reason;
    })
    .catch((value) => value);
...
test("withStream preserves an exact cancellation reason while consuming", async (t) => {
  ...
  const pending = harness.withStream(
    "binary",
    async ({ bytes }) => {
      for await (const _chunk of bytes.readExactly(1)) {
        /* consume */
