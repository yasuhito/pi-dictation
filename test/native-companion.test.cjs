...
const macOnly =
  process.platform === "darwin"
    ? {}
    : { skip: "requires the macOS Swift toolchain" };

test("production logout attribution observes confirmed loginwindow teardown", () => {
  const source = readFileSync(
    join(root, "native", "macos-companion", "PiDictationBridge.swift"),
    "utf8"
  );
  assert.equal(
    source.includes("com.apple.logoutContinued") &&
      source.includes("com.apple.logoutCancelled"),
    true
  );
});
...
test("production logout confirmation wins before console-lock attribution", () => {
  ...
  assert.match(
    source,
    /terminationRequest[\s\S]*logoutAttribution\.isContinued\(\)[\s\S]*failActive\(reason: "logout"\)[\s\S]*milliseconds\(750\)/
  );
});
...
test("production termination fallback leaves time for authoritative logout attribution", () => {
  ...
  assert.match(
    source,
    /terminationRequest[\s\S]*milliseconds\(750\)[\s\S]*failActive\(reason: "companion-stop"\)/
  );
});

test("the companion LaunchAgent bypasses post-login app-bundle xpcproxy stalls", () => {
  const source = readFileSync(
    join(root, "dist", "bin", "pi-dictation.js"),
    "utf8"
  );
  assert.match(
    source,
    /<string>\/bin\/sh<\/string><string>-c<\/string><string>exec &quot;\$1&quot;<\/string>/
  );
});

test(
  "production lifecycle wiring distinguishes logout from restart and shutdown",
  macOnly,
  () => {
    const directory = mkdtempSync(
      join(tmpdir(), "pi-dictation-lifecycle-wiring-")
    );
...
`
    ...
    try {
      ...
      if (compilation.status !== 0)
        throw new Error(compilation.stderr || compilation.stdout);
...
test(
  "production console-lock state maps IOKit Booleans and rejects absence",
  macOnly,
  () => {
...
test(
  "production input wiring gives capture and device-loss observation the same selected device",
  macOnly,
  () => {
    const directory = mkdtempSync(
      join(tmpdir(), "pi-dictation-device-wiring-")
    );
...
test(
  "production capture closes before the exact duration watchdog deadline",
  macOnly,
  () => {
    const directory = mkdtempSync(
      join(tmpdir(), "pi-dictation-capture-duration-")
    );
...
`
    ...
    try {
      ...
      if (execution.status !== 0)
        throw new Error(execution.stderr || execution.stdout);
      assert.equal(
        Number(execution.stdout.trim()) > 0 &&
          Number(execution.stdout.trim()) < 1,
        true
      );
    } finally {
      rmSync(directory, { recursive: true, force: true });
...
test(
  "duration watchdog requests only its exact parent then force-terminates it after the grace bound",
  macOnly,
  async (t) => {
    const directory = mkdtempSync(
      join(tmpdir(), "pi-dictation-watchdog-test-")
    );
    ...
    try {
      ...
      await t.test(
        "delivers the instance-bound termination request to its parent",
        () => {
          assert.equal(readFileSync(marker, "utf8").trim(), token);
        }
      );
      await t.test(
        "force-terminates that still-running parent after the testing grace bound",
        () => {
...
test(
  "failed-state persistence failure keeps audio until durable cleanup can retry",
  macOnly,
  async (t) => {
...
`
    ...
    try {
      ...
      await t.test(
        "does not remove audio before failed metadata is durable",
        () => {
