...
async function loadRecorder() {
  return jiti.import(join(packageRoot, "dist", "extensions", "recorder.js"));
}
...
test("Linux prefers pw-record when both recorders are available", async () => {
  ...
  const command = await defaultRecordCommand(
    "/tmp/voice.wav",
    environment("linux", ["pw-record", "arecord"])
  );
  assert.equal(
    command,
    "pw-record --format s16 --rate 16000 --channels 1 '/tmp/voice.wav'"
  );
});
...
test("Linux falls back to arecord", async () => {
  ...
  const command = await defaultRecordCommand(
    "/tmp/voice.wav",
    environment("linux", ["arecord"])
  );
  assert.equal(
    command,
    "arecord -q -f S16_LE -r 16000 -c 1 -t wav '/tmp/voice.wav'"
  );
});
...
test("recorder paths are shell quoted", async () => {
  const { defaultRecordCommand } = await loadRecorder();
  const command = await defaultRecordCommand(
    "/tmp/user's voice.wav",
    environment("linux", ["pw-record"])
  );
  assert.equal(
    command,
    "pw-record --format s16 --rate 16000 --channels 1 '/tmp/user'\\''s voice.wav'"
  );
});

test("missing local dependencies expose a stable safe classification", async (t) => {
  const { defaultRecordCommand } = await loadRecorder();
  const error = await defaultRecordCommand(
    "/private/user/voice.wav",
    environment("linux", [])
  ).catch((value) => value);
  await t.test("classifies the failure", () =>
    assert.equal(error.code, "recorder-unavailable")
  );
  await t.test("does not expose the destination", () =>
    assert.doesNotMatch(error.message, /private|voice\.wav/)
  );
});
...
test("the local Recorder commits a complete WAV only after stop succeeds", async (t) => {
  ...
  const recorder = createLocalRecorder({
    command: `${process.execPath} ${fixture} {file} --growing-wav`,
  });
  const observations = [];
  ...
  try {
    await t.test("keeps the destination absent while recording", () =>
      assert.equal(existsSync(destination), false)
    );
    await waitFor(() => observations.length > 0);
    await t.test("reports Level observations", () =>
      assert.equal(typeof observations[0].dbfs, "number")
    );
    await recording.stop();
    await t.test("commits a RIFF WAV", () =>
      assert.equal(readFileSync(destination, "ascii").slice(0, 4), "RIFF")
    );
    await t.test("makes repeated stop idempotent", () =>
      assert.doesNotReject(recording.stop())
    );
  } finally {
...
test("the local Recorder cancellation leaves no transcribable destination", async () => {
  const directory = mkdtempSync(
    join(tmpdir(), "pi-dictation-recorder-cancel-")
  );
  const destination = join(directory, "recording.wav");
  const { createLocalRecorder } = await loadRecorder();
  const recorder = createLocalRecorder({
    command: `${process.execPath} ${fixture} {file}`,
  });
  const recording = await recorder.start({
    destination,
    maxDurationMs: 10000,
...
for (const scenario of [
  ...
  test(`the local Recorder rejects ${scenario.name} without committing it`, async (t) => {
    const directory = mkdtempSync(
      join(tmpdir(), "pi-dictation-recorder-invalid-")
    );
    const destination = join(directory, "recording.wav");
    const { createLocalRecorder } = await loadRecorder();
    const recorder = createLocalRecorder({
      command: `${process.execPath} ${fixture} {file} ${scenario.argument}`,
    });
    const recording = await recorder.start({
      destination,
      maxDurationMs: 10000,
    ...
    try {
      ...
      await t.test("classifies invalid audio", () =>
...
test("startup abort prevents local external work", async () => {
  ...
  await assert.rejects(
    recorder.start({
      destination: "/private/destination.wav",
      maxDurationMs: 1000,
...
function contractHarness(recorder) {
  const directory = mkdtempSync(
    join(tmpdir(), "pi-dictation-recorder-contract-")
  );
  return {
    recorder,
    startOptions: {
...
runRecorderContract("local", async () => {
  ...
  return contractHarness(
    createLocalRecorder({ command: `${process.execPath} ${fixture} {file}` })
  );
});
