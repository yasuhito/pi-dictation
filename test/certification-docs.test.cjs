...
test("the actual npm tarball carries and executes its Bridge certification documentation", async (t) => {
  ...
  try {
    ...
    const extracted = spawnSync("tar", ["-xzf", tarball, "-C", temporary], {
      encoding: "utf8",
    });
    ...
    const listed = spawnSync(
      process.execPath,
      [
        join(packagedRoot, "dist", "bin", "pi-dictation-bridge-certify.js"),
        "list",
        "--json",
      ],
      {
    ...
    const certification = JSON.parse(listed.stdout);
    const scenarios = certification.scenarios.map(({ name }) => name);
    const readme = readFileSync(join(packagedRoot, "README.md"), "utf8");

    await t.test("ships the exact support boundary", () => {
      assert.equal(
        existsSync(join(packagedRoot, "docs", "bridge-recording-support.md")),
        true
      );
    });
    ...
    await t.test(
      "excludes mutable certification records from the immutable package bytes",
      () => {
        assert.equal(
          existsSync(
            join(
    ...
    await t.test(
      "reports the shared protocol version from packed bytes",
      () => {
        assert.equal(certification.protocolVersion, 3);
      }
    );
    await t.test("claims only the exact certified Mac and macOS build", () => {
      assert.match(
        readme,
        /Apple M1 Pro MacBook Pro \(`MacBookPro18,3`\) running macOS `26\.5\.1 \(25F80\)`/
      );
    });
  } finally {
