...
function canonicalHex(value, expectedBytes, stage = "response") {
  if (typeof value !== "string" || value.length !== expectedBytes * 2 || !/^[0-9a-f]+$/.test(value)) {
    throw failure("authentication", stage);
  }
...
function validateTiming(timing, keys) {
...
function validateInput(options, timingKeys) {
  ...
  validateTiming(options.timing, timingKeys);
...
function netConnection(endpoint) {
  ...
  return {
    ...
    read(maxBytes) {
      const available = socket.read(maxBytes);
      ...
      return new Promise((resolve, reject) => {
        ...
        const readable = () => {
          const chunk = socket.read(maxBytes);
...
class Reader {
  ...
  async chunk(maxBytes) {
    try {
      const chunk = await guarded(() => this.connection.read(maxBytes), this.policy, this.signal, this.stage, () => this.connection.destroy());
      if (chunk !== null) this.progress?.reset();
      return chunk;
    } catch (error) {
...
async function openExchange(options, timingKeys) {
  let payloadBytes;
  try {
    validateInput(options, timingKeys);
  ...
  try {
    ...
    if (challengeReader.buffer.length > 0) throw failure("malformed", "challenge");
    ...
    const reader = new Reader(connection, signal, timing.response, "response", timingKeys.includes("stream"));
    ...
    opened = true;
    return { connection, reader, status: response.status, payload: parsed, challenge, secret };
  } finally {
    if (!opened) connection.destroy();
  }
}

export async function request(options) {
  const exchange = await openExchange(options, ["connect", "challenge", "requestWrite", "response"]);
  try {
    await exchange.reader.end();
    return { status: exchange.status, payload: exchange.payload };
  } finally {
    exchange.connection.destroy();
  }
}
...
class ProgressDeadline {
  ...
  reset() {
    if (this.settled || this.policy.kind === "absolute" && this.timer !== undefined) return;
    clearTimeout(this.timer);
  ...
  run(operation) {
    if (this.settled) return this.interruption;
    if (this.policy.kind === "absolute" && this.policy.at <= Date.now()) {
      this.interrupt(failure("deadline", "stream"));
      return this.interruption;
    }
    return Promise.race([Promise.resolve().then(operation), this.interruption]);
  }
...
function binarySource(reader, progress) {
  let requested = false;
  let complete = false;
  let closed = false;
    ...
    source: {
      readExactly(length) {
        if (requested || !Number.isSafeInteger(length) || length < 0) {
          throw new TypeError("a non-negative exact byte count may be requested once");
        }
        requested = true;
        complete = length === 0;
...
function authenticatedFrames(reader, progress, exchange, options) {
  let iterated = false;
  let closed = false;
  let readFailure;
  const pending = new Set();
  const generator = (async function* () {
    for (let sequence = 0; ; sequence += 1) {
      ...
      if (!exactObject(message, ["type", "version", "requestId", "streamSequence", "payload", "hmac"]) ||
          ...
          typeof message.payload !== "string") throw failure("malformed", "stream");
      const payload = canonicalBase64(message.payload, undefined, "stream");
      const expected = tag(exchange.secret, [
        "stream", PROTOCOL_VERSION, PROTOCOL_VERSION, exchange.challenge, options.credential.id,
      ...
      const parsed = strictJson(payload, "stream");
      progress.reset();
      yield parsed;
    }
  })();
...
export async function withStream(options, consumer) {
  if (options?.kind !== "binary" && options?.kind !== "authenticated-frames") {
    throw failure("malformed", "request-write", new TypeError("invalid stream kind"));
  }
  if (typeof consumer !== "function") {
    throw failure("malformed", "request-write", new TypeError("stream consumer is required"));
  }
  const exchange = await openExchange(options, [
    "connect", "challenge", "requestWrite", "response", "stream", "end",
  ]);
  ...
  try {
    ...
    exchange.reader.policy = options.timing.stream;
    exchange.reader.stage = "stream";
    const progress = new ProgressDeadline(options.timing.stream, options.signal, exchange.connection);
    exchange.reader.progress = progress;
    try {
      const binary = options.kind === "binary" ? binarySource(exchange.reader, progress) : undefined;
      const authenticated = binary ? undefined : authenticatedFrames(exchange.reader, progress, exchange, options);
      const stream = binary
        ? { metadata: exchange.payload, bytes: binary.source }
        : { metadata: exchange.payload, frames: authenticated.source };
      let value;
      let consumerError;
      let consumerFailed = false;
      try {
        value = await progress.run(() => consumer(stream));
      } catch (error) {
        consumerError = error;
        consumerFailed = true;
      }
      ...
      if (binary) {
        ...
        progress.dispose();
        exchange.reader.policy = options.timing.end;
        await exchange.reader.end();
      }
      return { status: "ok", payload: exchange.payload, value };
    } finally {
      progress.dispose();
    }
  } finally {
