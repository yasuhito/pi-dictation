...
async function openAuthenticated(options, streaming = false) {
  ...
  try {
    ...
    const requestTag = tag(secret, [
      "request", protocolVersion, challenge, credential.id, requestId, operation, payloadBytes,
    ]);
    const bytes = framed({
      type: "request", version: protocolVersion, credentialId: credential.id, requestId, operation,
    ...
    const expected = tag(secret, [
      "response", protocolVersion, response.version, challenge, credential.id, requestId,
    ...
    if (response.status === "version-mismatch") {
      if (!exactObject(parsed, ["clientVersion", "companionVersion"]) ||
          parsed.clientVersion !== protocolVersion || parsed.companionVersion !== response.version ||
          response.version === protocolVersion) throw versionDataFailure("response");
    } else if (response.version !== protocolVersion) throw versionDataFailure("response");
    ...
    return {
      next: async () => {
        ...
        if (!exactObject(message, ["type", "version", "requestId", "streamSequence", "payload", "hmac"]) ||
            message.type !== "level-event" || message.version !== protocolVersion ||
        ...
        const expected = tag(this.secret, [
          "stream", protocolVersion, message.version, this.challenge, this.credentialId,
