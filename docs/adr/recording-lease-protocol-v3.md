...
The production capture path resolves the default input once, registers device-loss observation against that exact selected `AVCaptureDevice`, and verifies the default identity immediately before and after `AVAudioRecorder` starts (the strongest pinning that API exposes). Sleep and session-resign notifications fail capture. Because physical screen lock did not emit the session-resign notification on the certified macOS system, the companion also polls the IOKit root `IOConsoleLocked` Boolean at a fixed 250 ms interval while running and treats `true` as `session-lock`; the fixed timer is cancelled with the companion lifecycle. Distinct logout versus restart/shutdown Apple Events map to `logout` versus `reboot`; signal fallbacks preserve the same distinction. Companion stop, abrupt companion restart, and selected-device loss are independently attributable. Every lifecycle path deletes incomplete audio and never resumes it on wake, unlock, reconnect, restart, or device restoration.
