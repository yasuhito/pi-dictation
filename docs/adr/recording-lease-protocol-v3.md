...
The production capture path resolves the default input once, registers device-loss observation against that exact selected `AVCaptureDevice`, and verifies the default identity immediately before and after `AVAudioRecorder` starts (the strongest pinning that API exposes). Sleep and session-resign notifications fail capture. Distinct logout versus restart/shutdown Apple Events map to `logout` versus `reboot`; signal fallbacks preserve the same distinction. Companion stop, abrupt companion restart, and selected-device loss are independently attributable. Every lifecycle path deletes incomplete audio and never resumes it on wake, unlock, reconnect, restart, or device restoration.
