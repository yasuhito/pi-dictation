...
Pi Dictationは、OpenAIの音声文字起こしと、任意のローカル文字起こしコマンドに対応しています。失敗やキャンセルの後も外部コマンドがバックグラウンドに残ったり、リソースを使い続けたりしないよう、録音と文字起こしはそれぞれ上限を設けた別のプロセスグループで実行されます。キャンセル時やセッション終了時には、関連するプロセスをまとめて終了します。
